...
public class CompoundTag extends Tag<Map<String, Tag<?>>> implements Iterable<Map.Entry<String, Tag<?>>>, Comparable<net.querz.nbt.tag.CompoundTag>, MaxDepthIO {
	...
	public Set<Map.Entry<String, Tag<?>>> entrySet() {
		return new net.querz.nbt.tag.NonNullEntrySet<>(getValue().entrySet());
	}
	...
	public net.querz.nbt.tag.CompoundTag getCompoundTag(String key) {
		return get(key, net.querz.nbt.tag.CompoundTag.class);
	}
	...
	public boolean equals(Object other) {
		...
		if (!super.equals(other) || size() != ((net.querz.nbt.tag.CompoundTag) other).size()) {
			return false;
		}
		for (Map.Entry<String, Tag<?>> e : getValue().entrySet()) {
			Tag<?> v;
			if ((v = ((net.querz.nbt.tag.CompoundTag) other).get(e.getKey())) == null || !e.getValue().equals(v)) {
				return false;
			}
		}
		return true;
	...
	@Override
	public int compareTo(net.querz.nbt.tag.CompoundTag o) {
		return Integer.compare(size(), o.getValue().size());
	}

	@Override
	public net.querz.nbt.tag.CompoundTag clone() {
		net.querz.nbt.tag.CompoundTag copy = new net.querz.nbt.tag.CompoundTag();
		for (Map.Entry<String, Tag<?>> e : getValue().entrySet()) {
