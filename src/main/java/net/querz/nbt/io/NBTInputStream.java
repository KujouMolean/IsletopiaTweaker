...
public class NBTInputStream extends DataInputStream implements NBTInput, MaxDepthIO {
	...
	private static Map<Byte, ExceptionBiFunction<net.querz.nbt.io.NBTInputStream, Integer, ? extends Tag<?>, IOException>> readers = new HashMap<>();
	...
	static {
		...
		put(ListTag.ID, net.querz.nbt.io.NBTInputStream::readListTag, ListTag.class);
		put(CompoundTag.ID, net.querz.nbt.io.NBTInputStream::readCompound, CompoundTag.class);
		put(IntArrayTag.ID, (i, d) -> readIntArray(i), IntArrayTag.class);
		put(LongArrayTag.ID, (i, d) -> readLongArray(i), LongArrayTag.class);
	}

	private static void put(byte id, ExceptionBiFunction<net.querz.nbt.io.NBTInputStream, Integer, ? extends Tag<?>, IOException> reader, Class<?> clazz) {
	...
	private Tag<?> readTag(byte type, int maxDepth) throws IOException {
		ExceptionBiFunction<net.querz.nbt.io.NBTInputStream, Integer, ? extends Tag<?>, IOException> f;
	...
	private static ByteTag readByte(net.querz.nbt.io.NBTInputStream in) throws IOException {
		return new ByteTag(in.readByte());
	}

	private static ShortTag readShort(net.querz.nbt.io.NBTInputStream in) throws IOException {
		return new ShortTag(in.readShort());
	}

	private static IntTag readInt(net.querz.nbt.io.NBTInputStream in) throws IOException {
		return new IntTag(in.readInt());
	}

	private static LongTag readLong(net.querz.nbt.io.NBTInputStream in) throws IOException {
		return new LongTag(in.readLong());
	}

	private static FloatTag readFloat(net.querz.nbt.io.NBTInputStream in) throws IOException {
		return new FloatTag(in.readFloat());
	}

	private static DoubleTag readDouble(net.querz.nbt.io.NBTInputStream in) throws IOException {
		return new DoubleTag(in.readDouble());
	}

	private static StringTag readString(net.querz.nbt.io.NBTInputStream in) throws IOException {
		return new StringTag(in.readUTF());
	}

	private static ByteArrayTag readByteArray(net.querz.nbt.io.NBTInputStream in) throws IOException {
		ByteArrayTag bat = new ByteArrayTag(new byte[in.readInt()]);
		in.readFully(bat.getValue());
		return bat;
	}

	private static IntArrayTag readIntArray(net.querz.nbt.io.NBTInputStream in) throws IOException {
		int l = in.readInt();
		int[] data = new int[l];
	...
	private static LongArrayTag readLongArray(net.querz.nbt.io.NBTInputStream in) throws IOException {
		int l = in.readInt();
	...
	private static ListTag<?> readListTag(net.querz.nbt.io.NBTInputStream in, int maxDepth) throws IOException {
	...
	private static CompoundTag readCompound(net.querz.nbt.io.NBTInputStream in, int maxDepth) throws IOException {
