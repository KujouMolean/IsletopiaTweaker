...
public class PlayTimeStatisticsDao {
    ...
    public static void addRecord(String player, String server, long start, long end) {
        try (Connection connection = DataSourceUtils.getConnection()) {
            String sql = "insert into minecraft.playtime_statistics(player, server, startTimeStamp, endTimeStamp, playtime) VALUES (?,?,?,?,?)";
            PreparedStatement preparedStatement = connection.prepareStatement(sql);
    ...
    public static long getLastPlayTimestamp(String player){
        try (Connection connection = DataSourceUtils.getConnection()) {
            String sql = "select startTimeStamp  from minecraft.playtime_statistics where player=?\n" +
    ...
    public static long getRecentPlayTime(String player,long start){
        try (Connection connection = DataSourceUtils.getConnection()) {
            String sql = "select SUM(playtime) from minecraft.playtime_statistics where player=? and startTimeStamp>?;";
            PreparedStatement preparedStatement = connection.prepareStatement(sql);
            preparedStatement.setString(1, player);
