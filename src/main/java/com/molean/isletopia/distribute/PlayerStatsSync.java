...
public class PlayerStatsSync implements Listener {
    ...
    public PlayerStatsSync(PlayerManager playerManager) {
        ...
        stringDataLoadTask.setAsyncLoad(this::asyncLoad);
        stringDataLoadTask.setSyncRestore(this::syncRestore);
        playerManager.registerDataLoading(stringDataLoadTask);
        playerManager.registerRoundUpdateTask(this::roundUpdateTask, 20);
        playerManager.registerQuitUpdateTask(this::quitTask);
    }

    public void asyncLoad(Player player, Consumer<String> consumer) {
        try {
    ...
    public void syncRestore(Player player, String s, Consumer<Player> consumer) {
        StatsSerializeUtils.loadStats(player, s);
        consumer.accept(player);
    }

    public void roundUpdateTask(Player player) {
